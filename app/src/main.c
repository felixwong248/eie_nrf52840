...
static int parse_wav(const char *path)
{
    ...
    off_t data_offset = -1; // data offset thats used to return to data subchunk
    ...
    while (1) {
        ...
        if (memcmp(sub_chunk_id, "fmt ", 4) == 0) {
            ...
            audio_format    = u16little_en(&fmt_buff[0]); // = 1 for PCM
            num_channels    = u16little_en(&fmt_buff[2]);
            sample_rate     = u32little_en(&fmt_buff[4]);
            byte_rate       = u32little_en(&fmt_buff[8]);
            block_align     = u16little_en(&fmt_buff[12]);
            bits_per_sample = u16little_en(&fmt_buff[14]);

            got_fmt = true;
        } else if (memcmp(sub_chunk_id, "data", 4) == 0) { // data subchunk contains actual song
            data_offset = fs_tell(&f); // fs_tell gives current position of file pointer
